...
try:
    from TEXT_DETECTION import detect_local_models
except ImportError as e:
    print(f"Import error: {e}")
    detect_local_models = None
...
def analyzeTextArtifact():
    data = request.get_json()
    text = data.get('text', '').strip()
    
    if not text or len(text.split()) < 20:
        return jsonify({"error": "Text must be at least 20 words for reliable analysis."}), 400

    try:
        report, error = detect_local_models.get_log_likelihood(text)
